...
# MacAdam 1942 25 色参考 (1931 xy 中心 + 半长/半短轴 + 长轴角). 代表性子集.
_MACADAM_25 = [
    (0.469, 0.395, 0.0012, 0.0008, 20.0), (0.547, 0.396, 0.0016, 0.0010, 15.0),
    (0.593, 0.368, 0.0018, 0.0010, 10.0), (0.569, 0.425, 0.0020, 0.0012, 30.0),
    (0.515, 0.417, 0.0015, 0.0010, 25.0), (0.490, 0.445, 0.0013, 0.0009, 18.0),
    (0.333, 0.377, 0.0011, 0.0007, 0.0),  (0.390, 0.422, 0.0012, 0.0008, 22.0),
    (0.365, 0.458, 0.0010, 0.0007, 5.0),  (0.344, 0.473, 0.0010, 0.0006, 0.0),
    (0.399, 0.505, 0.0010, 0.0007, 8.0),  (0.415, 0.517, 0.0010, 0.0007, 12.0),
    (0.417, 0.553, 0.0011, 0.0007, 15.0), (0.478, 0.518, 0.0012, 0.0008, 20.0),
    (0.490, 0.523, 0.0013, 0.0008, 18.0), (0.493, 0.533, 0.0013, 0.0008, 20.0),
    (0.398, 0.494, 0.0010, 0.0007, 10.0), (0.375, 0.471, 0.0010, 0.0006, 6.0),
    (0.345, 0.351, 0.0012, 0.0008, 5.0),  (0.275, 0.350, 0.0015, 0.0009, 0.0),
    (0.260, 0.360, 0.0016, 0.0010, 0.0),  (0.304, 0.373, 0.0014, 0.0009, 5.0),
    (0.240, 0.266, 0.0020, 0.0012, 0.0),  (0.225, 0.275, 0.0022, 0.0013, 0.0),
    (0.213, 0.285, 0.0024, 0.0014, 0.0),
]


def set_macadam_table(table) -> None:
    """替换默认 25 色 MacAdam 表 (实测子集). 每项 (x,y,a,b,theta_deg)."""
    global _MACADAM_25
    _MACADAM_25 = list(table)


def _uv_jacobian_scale(x, y):
    """xy -> u'v' 局部缩放 (近似: 沿 x 的单位位移在 u'v' 的大小)."""
    D = -2.0 * x + 12.0 * y + 3.0
    if abs(D) < 1e-12:
        return 1.0
    du_dx = 4.0 / D - 4.0 * x * (-2.0) / (D * D)
    dv_dx = 9.0 * y * (-2.0) / (D * D)
    s = math.sqrt(du_dx * du_dx + dv_dx * dv_dx)
    return s if s > 1e-9 else 1.0


def macadam_ellipse_for_color(x, y, table=None) -> MacAdamEllipse:
    """按参考色 (xy) 从 25 色表选取最近 MacAdam 椭圆, 中心为该色 u'v'.

    半轴由表中 xy 轴换算到 u'v' (局部 Jacobian 缩放), 长轴角沿用.
    """
    tab = _MACADAM_25 if table is None else table
    u, v = uv_prime(x, y)
    best = min(tab, key=lambda e: (e[0] - x) ** 2 + (e[1] - y) ** 2)
    _bx, _by, a_xy, b_xy, theta = best
    s = _uv_jacobian_scale(x, y)
    return MacAdamEllipse(u, v, a_xy * s, b_xy * s, theta)
