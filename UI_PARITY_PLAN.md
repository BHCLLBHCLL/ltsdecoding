...
> **命令实现化进度（2026-08-25）**：菜单注册表（`ui_command_map.json`）202 项 —— **202 implemented / 0 NYI**（205 个注册 handler，命令清零）。含：Photoreal 占位真实化（pr_view 快照/相机/点·聚光灯/begin_lit/render_after_lit）、混合仿真（begin_bwd）、SetupRTMode 开关、CPC 剖面实体、CODE V 导入导出（.seq 往返）、未删除（Undelete 恢复）、CATIA/Parasolid/DXF 交换路由、Addins/SW Link/Recent 列表、批量编辑、Imaging Path 视图。
