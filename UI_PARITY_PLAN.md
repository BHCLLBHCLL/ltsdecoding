...
> **命令实现化进度（2026-08-25）**：菜单注册表（`ui_command_map.json`）202 项 —— **202 implemented / 0 NYI**（205 个注册 handler，命令清零）。
> **物理深度·偏振（2026-08-25）**：`ltsoptics/polarization.py`（Jones 向量/复菲涅尔 s-p/斯托克斯/Malus·Brewster·TIR·检偏矩阵/Poincaré 角）+ `lts/trace/polarization_report.py`（平均 Stokes/DOP）；`surface_event`/`fresnel_interface_event` 支持 `jones`（圆偏振光过 TIR 界面 -> 椭圆态 S3≠0、DOP=1，解析验证）。
> **工程化收口（2026-08-25）**：`lts/golden.py` SQLite 黄金回归（`tests/goldens.db`，16 项确定性基准：顺序成像 f/BFL/spot、优化器 Rosen/GA、MACRO、apodizer 矩、玻璃 Nd/Vd、区分类、CODE V 往返；`tests/test_golden.py` 漂移即失败，`verify_goldens.py --seed` 重建）；常驻校验脚本：`verify_ui.py`（13 菜单/202 项/0 NYI/136 别名）、`verify_goldens.py`、`verify_pipeline.py`（rearlighting 解析→绑定→写回→重解析闭环，EXIT 0）、`verify_raytrace.py`（通量守恒 0.0003%、LT 窗口峰值命中）、`verify_all.py`（一键；`--full` 含重任务）。含：Photoreal 占位真实化（pr_view 快照/相机/点·聚光灯/begin_lit/render_after_lit）、混合仿真（begin_bwd）、SetupRTMode 开关、CPC 剖面实体、CODE V 导入导出（.seq 往返）、未删除（Undelete 恢复）、CATIA/Parasolid/DXF 交换路由、Addins/SW Link/Recent 列表、批量编辑、Imaging Path 视图。
